...
const MenuCategoryMaster = () => {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true)
      setError('')
      const res = await axios.get('/MenuCategoryMasters', {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError('')
      setSuccess('')
      await axios.delete(`/MenuCategoryMasters/${id}`)
      setSuccess('Menu category deleted')
      await fetchItems()
      setTimeout(() => setSuccess(''), 3000)
    } catch (e) {
      console.error('Error deleting menu category:', e)
      setError(e.response?.data?.message || 'Failed to delete menu category')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`/MenuCategoryMasters/${editingId}`, payload)
        setSuccess('Menu category updated')
      } else {
        await axios.post('/MenuCategoryMasters', payload)
      ...
      console.error('Error saving menu category:', e2)
      setError(e2.response?.data?.message || 'Failed to save menu category')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-amber-600 to-yellow-700 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold mb-2">Menu Category Master</h1>
            <p className="text-amber-100">Manage menu categories used in Menu Item Master</p>
          </div>
          <TagIcon className="h-12 w-12 text-amber-200" />
        </div>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">{editingId ? 'Edit Category' : 'Add Category'}</h2>
          </div>
