...
const MenuItemMaster = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Error saving menu item. Please try again.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-amber-600 to-yellow-700 rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold mb-2">Menu Item Master</h1>
            <p className="text-amber-100">Manage restaurant menu items and recipes</p>
          </div>
          <DocumentTextIcon className="h-12 w-12 text-amber-200" />
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                {menuItems.filter(item => item.isSpicy).length}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">
                Rs {menuItems.length ? Math.round(menuItems.reduce((sum, item) => sum + (Number(item.price) || 0), 0) / menuItems.length) : 0}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="flex items-center px-4 py-2 bg-gradient-to-r from-amber-600 to-yellow-600 text-white font-semibold rounded-lg hover:from-amber-700 hover:to-yellow-700 transition-all duration-200"
        >
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-8 py-2 bg-gradient-to-r from-amber-600 to-yellow-600 text-white font-semibold rounded-lg hover:from-amber-700 hover:to-yellow-700 disabled:opacity-50 transition-all duration-200 flex items-center"
              >
                {loading ? (
                  <>
                ...
                <tr key={getItemId(item)} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      item.category === 'Main Course' ? 'bg-red-100 text-red-800' :
                      item.category === 'Appetizers' ? 'bg-orange-100 text-orange-800' :
