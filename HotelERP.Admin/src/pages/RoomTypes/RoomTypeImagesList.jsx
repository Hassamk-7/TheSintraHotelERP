...
import { HomeIcon, PhotoIcon, EyeIcon, CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline'

const RoomTypeImagesList = () => {
  ...
  // Load room types and their image counts from database
  useEffect(() => {
    fetchRoomTypesWithImageCounts()
  }, [])

  // Fetch room types with image counts - DYNAMIC FROM DATABASE
  const fetchRoomTypesWithImageCounts = async () => {
    try {
      setLoading(true)
      setError('')
      ...
      const roomTypesWithImageCounts = await Promise.all(
        roomTypesData.map(async (roomType) => {
          try {
            console.log('🚀 Fetching images for room type:', roomType.id)
            ...
            // If 404, means no images for this room type
            ...
            console.error('❌ Error fetching images for room type:', roomType.id, err)
            return {
              ...roomType,
              imageCount: 0,
              thumbnailImage: null
      ...
      setRoomTypes(roomTypesWithImageCounts)
      console.log('✅ Room types with image counts:', roomTypesWithImageCounts)
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <div className="flex items-center text-sm text-gray-500">
            ...
            <span className="mx-2">/</span>
            <span className="text-gray-900 font-medium">Image Gallery</span>
          </div>
        </div>
      </div>
        ...
        <div className="px-6 py-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-100">Manage image galleries for each room type</p>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">Create room types first to manage their images</p>
            <button
          ...
          {roomTypes.map((roomType) => (
            <div key={roomType.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
              <div className="h-40 bg-gradient-to-br from-purple-100 to-purple-200 flex items-center justify-center relative overflow-hidden">
                {roomType.thumbnailImage && (roomType.thumbnailImage.thumbnailPath || roomType.thumbnailImage.imagePath) ? (
                  <>
                    <img 
                      ...
                      alt={roomType.thumbnailImage.altText || `${roomType.name} thumbnail`}
                      className="w-full h-full object-cover hover:scale-105 transition-transform duration-300"
                      onError={(e) => {
                        console.log('❌ Thumbnail failed to load:', roomType.thumbnailImage.thumbnailPath || roomType.thumbnailImage.imagePath)
                        e.target.style.display = 'none'
                        e.target.parentElement.querySelector('.fallback-icon').style.display = 'flex'
                      }}
                    />
                    {roomType.thumbnailImage.isPrimary && (
                ...
                <div className={`fallback-icon absolute inset-0 flex items-center justify-center ${roomType.thumbnailImage && (roomType.thumbnailImage.thumbnailPath || roomType.thumbnailImage.imagePath) ? 'hidden' : 'flex'}`}>
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center mb-4">
                  <p className="text-sm text-gray-600">
                    {roomType.imageCount} {roomType.imageCount === 1 ? 'image' : 'images'}
                  </p>
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                    roomType.imageCount > 0 
                      ? 'bg-green-100 text-green-800' 
                      : 'bg-gray-100 text-gray-800'
                  }`}>
                    {roomType.imageCount > 0 ? 'Has Images' : 'No Images'}
                  </span>
                </div>
                <button
                  onClick={() => navigate(`/rooms-management/room-types/${roomType.id}/images`)}
                ...
                >
                  ...
                  <span>
                    {roomType.imageCount > 0 ? 'View Images' : 'Add Images'}
                  </span>
                </button>
              </div>
            </div>
...
export default RoomTypeImagesList
